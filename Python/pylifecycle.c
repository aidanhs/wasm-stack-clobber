...
_Py_InitializeEx_Private(int install_sigs, int install_importlib)
{
    ...
    PyObject_Print(interp->builtins, stderr, 0);
    fprintf(stderr, "\n");
    PyObject_Print(interp->builtins, stderr, 0);
    fprintf(stderr, "\n");
    ...
    _PyExc_Init(bimod);
    fprintf(stderr, "here we go 6\n");
    PyObject_Print(interp->builtins, stderr, 0);
    fprintf(stderr, "\n");

    sysmod = _PySys_Init();
    fprintf(stderr, "here we go 6.14\n");
    if (sysmod == NULL)
        Py_FatalError("Py_Initialize: can't initialize sys");
    PyObject_Print(interp->builtins, stderr, 0);
    fprintf(stderr, "\n");
    fprintf(stderr, "here we go 6.15\n");
    interp->sysdict = PyModule_GetDict(sysmod);
    fprintf(stderr, "here we go 6.2\n");
    ...
    PyObject_Print(interp->builtins, stderr, 0);
    fprintf(stderr, "\n");
    fprintf(stderr, "here we go 7\n");
    _PyImport_Init();

    fprintf(stderr, "here we go 7.1\n");
    _PyImportHooks_Init();

    /* Initialize _warnings. */
    fprintf(stderr, "here we go 7.2\n");
    _PyWarnings_Init();

    if (!install_importlib)
        return;

    fprintf(stderr, "here we go 7.3\n");
    if (_PyTime_Init() < 0)
        Py_FatalError("Py_Initialize: can't initialize time");

    fprintf(stderr, "here we go 8\n");
    import_init(interp, sysmod);
    fprintf(stderr, "here we go 9\n");
