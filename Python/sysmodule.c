...
#define SET_SYS_FROM_STRING(key, value)                    \
    ...
    fprintf(stderr, "here we go 6.05\n");
    SET_SYS_FROM_STRING_BORROW("__displayhook__",
                               PyDict_GetItemString(sysdict, "displayhook"));
    fprintf(stderr, "here we go 6.051\n");
    SET_SYS_FROM_STRING_BORROW("__excepthook__",
                               PyDict_GetItemString(sysdict, "excepthook"));
    fprintf(stderr, "here we go 6.052\n");
    SET_SYS_FROM_STRING("version",
                         PyUnicode_FromString(Py_GetVersion()));
    fprintf(stderr, "here we go 6.053\n");
    SET_SYS_FROM_STRING("hexversion",
                         PyLong_FromLong(PY_VERSION_HEX));
    fprintf(stderr, "here we go 6.054\n");
    SET_SYS_FROM_STRING("_git",
                        Py_BuildValue("(szz)", "CPython", _Py_gitidentifier(),
                                      _Py_gitversion()));
    fprintf(stderr, "here we go 6.055\n");
    SET_SYS_FROM_STRING("dont_write_bytecode",
                         PyBool_FromLong(Py_DontWriteBytecodeFlag));
    fprintf(stderr, "here we go 6.056\n");
    SET_SYS_FROM_STRING("api_version",
                        PyLong_FromLong(PYTHON_API_VERSION));
    fprintf(stderr, "here we go 6.057\n");
    SET_SYS_FROM_STRING("copyright",
                        PyUnicode_FromString(Py_GetCopyright()));
    fprintf(stderr, "here we go 6.058\n");
    SET_SYS_FROM_STRING("platform",
                        PyUnicode_FromString(Py_GetPlatform()));
    fprintf(stderr, "here we go 6.059\n");
    Py_GetProgramFullPath();
    fprintf(stderr, "here we go 6.05901\n");
    SET_SYS_FROM_STRING("executable",
                        PyUnicode_FromWideChar(
                               Py_GetProgramFullPath(), -1));
    fprintf(stderr, "here we go 6.0591\n");
    SET_SYS_FROM_STRING("prefix",
                        PyUnicode_FromWideChar(Py_GetPrefix(), -1));
    fprintf(stderr, "here we go 6.0592\n");
    SET_SYS_FROM_STRING("exec_prefix",
                        PyUnicode_FromWideChar(Py_GetExecPrefix(), -1));
    fprintf(stderr, "here we go 6.0593\n");
    SET_SYS_FROM_STRING("base_prefix",
                        PyUnicode_FromWideChar(Py_GetPrefix(), -1));
    fprintf(stderr, "here we go 6.0594\n");
    SET_SYS_FROM_STRING("base_exec_prefix",
                        PyUnicode_FromWideChar(Py_GetExecPrefix(), -1));
    fprintf(stderr, "here we go 6.0595\n");
    SET_SYS_FROM_STRING("maxsize",
                        PyLong_FromSsize_t(PY_SSIZE_T_MAX));
    fprintf(stderr, "here we go 6.0596\n");
    SET_SYS_FROM_STRING("float_info",
                        PyFloat_GetInfo());
    fprintf(stderr, "here we go 6.0597\n");
    SET_SYS_FROM_STRING("int_info",
                        PyLong_GetInfo());
    fprintf(stderr, "here we go 6.08\n");
    ...
    fprintf(stderr, "here we go 6.09\n");
    ...
    if (res < 0 && PyErr_ExceptionMatches(PyExc_KeyError))
        PyErr_Clear();
    fprintf(stderr, "here we go 6.095\n");
...
#undef SET_SYS_FROM_STRING_BORROW
    fprintf(stderr, "here we go 6.099\n");
    if (PyErr_Occurred())
        return NULL;
    return m;
}
